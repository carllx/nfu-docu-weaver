...
从 v1.4.0 开始，Docu-Weaver 引入了 **Schema-Driven Architecture**（模式驱动架构），将数据契约（Data Contract）提升为系统的第一级架构组件。v2.0 进一步强化了这一理念，实现了 **Schema / Template / Data 三层完全分离**，确保从 AI 数据生成、数据验证到文档合成的全流程一致性和可靠性。
...
### v1.4-v2.0: Schema 驱动阶段 (Schema-Driven)
...
nfu-docu-weaver/
│
├── 📂 schemas/                    # 🏛️ Layer 1 - Schema层（数据契约）
│   ├── README.md                 # Schema 使用指南
│   ├── course_schema.yml         # Course配置 Schema (v2.0新增)
│   ├── lesson_schema.yml         # Lesson数据 Schema
│   ├── outline_schema.yml        # Outline数据 Schema
│   └── cover_schema.yml          # Cover数据 Schema
│
├── 📂 templates/                  # 📄 Layer 2 - Template层（呈现）
│   ├── lesson/
│   │   └── lesson.docx
│   ├── cover/
│   │   └── cover.docx
...
│
├── 📂 agents/                    # 🤖 Agent层
│   └── lesson-weaver.md          # Lesson-Weaver Agent (v2.0升级)
│
├── 📂 tools/                     # 🔧 工具层
│   └── course_manager.py         # Course管理工具 (v2.0升级)
│
└── 📂 output/                    # 📁 输出层
```
