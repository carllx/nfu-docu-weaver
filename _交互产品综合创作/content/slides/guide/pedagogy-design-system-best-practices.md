...
4. **【第三轮交互】定义品牌文法 (Brand Grammar):**
   * **你的指令 (User Input):**  
     ...
     > 
     > **视觉文法 (Visual Grammar):**
     > - **粗描线 (Thick Outlines):** 所有元素边缘使用粗黑色描边（2px），模拟手绘墨线
     ...
     > - **卡通化字体 (Cartoonish Fonts):** 使用圆润、粗体、手写风格字体（如 Fredoka, Nunito）
     > - **夸张对比 (Exaggerated Contrast):** 标题极粗（700 weight），正文适中（400 weight）"
...
- [ ] **品牌文法** 清晰描述视觉、动效、字体的具体特征（含可量化参数，如 "2px 描边"）
...
4. **【第三轮交互 - 核心】定义 Primitive Tokens (Reference Layer):**
   * **你的指令 (User Input):**  
     ...
     > - `color.primitive.red.500: #FF6B6B`（点缀色）
     ...
     > 
     > **Stroke Width (Primitive):** 基于品牌文法 ('粗描线 2px')：
     > - `stroke.primitive.sm: 1px`
     > - `stroke.primitive.md: 2px`
     > - `stroke.primitive.lg: 2.5px`
     > 
     > **Typography (Primitive):** 基于品牌文法 ('圆润、粗体、手写风格')：
     > - `font.primitive.family.primary: 'Fredoka', sans-serif`
     > - `font.primitive.family.secondary: 'Nunito', sans-serif`
     ...
     > - `easing.primitive.elastic: cubic-bezier(0.2, 0.8, 0.2, 1)`
     > - `easing.primitive.smooth: cubic-bezier(0.4, 0, 0.2, 1)`
     > 
     > **Duration (Primitive):**
     > - `duration.primitive.fast: 120ms`
     > - `duration.primitive.normal: 180ms`
     > 
...
6. **【第五轮交互】定义组件规范 (Component Specifications):**
   * **你的指令 (User Input):**  
     ...
     > - Transition: `{animation.semantic.button.hover.duration}` `{animation.semantic.button.hover.easing}` → `180ms cubic-bezier(0.2, 0.8, 0.2, 1)`
     > ```
     > 
...
**精确值 (Values):**
- [ ] 所有 Tokens 都有具体、可执行的值（如 `#FF6B6B`, `16px`, `'Fredoka', sans-serif`, `cubic-bezier(0.2, 0.8, 0.2, 1)`）
...
- [ ] Stroke: 2px 粗描边
- [ ] Radius: 8-16px 圆角
- [ ] Typography: 圆润粗体字体（Fredoka/Nunito）、夸张对比（700 vs 400 weight）
...
4. **【第三轮交互 - 核心】生成 CSS Variables 完整清单:**
   * **你的指令 (User Input):**  
     ...
     >   --color-primitive-gray-500: #9CA3AF;
     >   --color-primitive-gray-900: #1F2937;
     >   --color-primitive-red-500: #FF6B6B;
...
4. **验证与检查 (在 Figma 中):**
   ...
   **Text Styles:**
   ...
   - [ ] 字体族、大小、字重、行高与规范一致
   - [ ] 使用的字体（Fredoka, Nunito）在 Figma 中可用
