...
class DocumentGenerator:
    ...
    def batch_generate(self, data_dir, template_path, output_dir, continue_on_error=False, debug=False, quiet=False, verbose=False):
        ...
        # 使用 tqdm 进度条或普通循环
        iterator = tqdm(yaml_files, desc="批量生成文档", unit="file", disable=not use_progress_bar) if use_progress_bar else yaml_files
        
        for idx, data_file in enumerate(iterator if use_progress_bar else yaml_files, 1):
