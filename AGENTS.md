...
**【核心红线】**：在执行当前路由及一般项目任务期间，**绝对禁止修改 `01_ARCHITECTURE_SPEC.md` 和 `08_HANDOFF_BUNDLE.md`**。如遇必须调整的情形，只能在 Decision Log 中写入 Proposed 草案，等待用户审核。
...
对于你输出的所有重要陈述，必须**严格遵循** `02_EPISTEMIC_RULES.md` 中的“内容身份”标签规范进行标识（不在此重复定义）。
...
- **输出要求**：立即生成临时交接文档包（记录当前目标、阶段、已完成项、未决问题与下一步任务），**严禁直接修改正式的 `08_HANDOFF_BUNDLE.md`**。
