...
我现在准备要备一个课，是在玛雅的建模课里面，要向同学们讲解灯光和色彩，就是在材质上面运用，以一个家居，一个办公室的一个道具。为例，就给这个空间打光，以及给它赋予颜色，给这些模型的各种部件。然后我这里面涉及到有些原则，比如说我的上一节课已经讲了一些材质比较一个物理 BPR 的物理材质的渲染。




然后这节课呢，我会再讲，也从灯光讲起，因为灯光是素描关系，重于素素描关系的一个一个手段。那么在这个素描关系在一整个渲染的构图里面是最重要的，因为色彩尽管在高考的时候，就是画，考美院的时候画色彩呢，也是更重要的是看评分，是看素描关系。然后色彩呢它是其到跟，也是很重要。但是素描关系是基础了，色彩关系呢它是它重要呢是能够直接调动观众的情感。然后在现在我的这个课程里面呢，在前面讲的这个颜，灯光呢以三三点灯光为为主，这3点布光的原理呢，我会有很有比较多的这种方案，就是有不同的摆放3点布光的方案，那么这个方方案的信息我会在后面补充，不同的方案的信息会在后面补充。然后在在这里面三点布光呢，主要是有一个是主光 key light，还有那个 fill light 就是还有一个填充灯光，还有第三个是那个 rim light 就是一个是主灯光，一个是给灯光，给现场气氛调节对比啊深度的灯光，然后第三个就是轮廓光。那么这三个轮，这三个光，它的调节呢是第二个灯光呢是给决定画面的对比程度的，第三个是取决于结构，就是我们的轮廓光是取决于我们的要表现现场道具的结构的，然后它也的特征有三要素。数，就是这三个灯光的比例调节。那么接着我会讲怎么样布色，就是怎么样去给这些场景的道具涂色。因为前面的这个素描关系要定下来，素描关系就最好不要上颜色，在我的角度来说。最好是模型都是纯白的。然后接着来说我们的第 4 上色的时候呢，我会推荐用  60%、30%、10% 的法则这种比例的，这种规则的一种上色的原则，让学生们去实践。这三种颜色，它这种，这个规则是应用于很多方面，除了我们的室内的设计，还应用于我们的 UI 设计，以及我们的衣服，就是我们穿搭，服饰的穿搭设计上面。那么这三个它跟灯光也很相似，就是比如说 60% 呢是主主色调，30% 是它的辅助主色调。它就像我们前面的那个主灯光和我们的第二次要灯光，它也是，第二次要灯光是去决定我们整个画面的它的对比的。那所以我们这个颜色也是的，次要颜色也是，它就是也也是取决于现场环境的柔和程度来辅助我们主色调。那么他们在这里面我也要讲讲这第三个颜颜色呢，一般就是点缀的颜色了。然后这三个颜色我会结合我需要结合我们的三个灯光来去联系一起讲，然后让同学们有一个对应的关系。那么这个我也会给一些案例，就是分别是室内的，还有 UI 的，还有穿搭的这个案例给同学们来讲。在布光在玛雅上面我会进行演示，这个演示的过程，在上色的时候呢我会把那个灯光去掉，就是用玛雅的只显示颜色的这个模式去去上色。然后接着后面就是学生的演示环节，最后其实我在上完色之后，这上上色是因为上的颜色主要是给同同学们讲的这个颜色的规则是为了让学生们不要乱上颜色，因为这样乱上颜色很难管理。有了这个规则呢，同学们的结果呢会模型连接的材质球会更少，然后这样很方便管理，然后也方便我们后面呢把模型这个场景上传到那个 sketchfab 这个平台上面。然后上传到 sketchfab 平台上面之后，我们也很好的去控制我们的材质，就比如说我们要客户不满意，我们随时可以调整方案，我们就可以就是更改更少的材质球，来去改动我们的设计，就使得我们改就是迭代或者更新优化的过程，也就是会更加科学、更高效了。那最后的环节就是教同学们去把我们的模型场景上传到 sketchfab  。
