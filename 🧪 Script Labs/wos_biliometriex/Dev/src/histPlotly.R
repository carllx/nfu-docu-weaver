# Implement the following code with Plotly's R graphing library
# ```
# geom_network_edges
# ```
# (-- `bibliometrix/R/histPlot.R at 2f2c03b9bc0068992af66940a6e9399935b5137a · massimoaria/bibliometrix` [github](https://github.com/massimoaria/bibliometrix/blob/2f2c03b9bc0068992af66940a6e9399935b5137a/R/histPlot.R#L3))
...
histPlotly <- function(histResults, n = 20, size = 5, labelsize = 5, remove.isolates = TRUE, title_as_label = FALSE, label = "short", verbose = TRUE) {
    # Color palette setup
    colorlist <- c(
    ...
    layout_norm <- layout_m %>%
        mutate(
            x = (.data$x - min(.data$x)) / (max(.data$x) - min(.data$x)),
    ...
    g <- ggplot(df_net, aes(x = .data$x, y = .data$y, xend = .data$xend, yend = .data$yend, text = .data$text)) +
        ...
        geom_text(aes(label = .data$id, color = .data$color_v),
            size = labelsize,
            nudge_x = 0,
            nudge_y = 0.02,
